...
SYSTEM_PROMPT = {
    ...
    "extraction_rules": {
        "ko": (
            ...
            "- **질문이 좁고 구체적**일 때만(예: 이미 언급된 이슈에 대해 위치/빈도/영향 대상/심각도 등 세부사항을 캐묻는 후속 질문, "
            "또는 응답자가 이미 방향을 제시해서 몇 가지 전형적 사례로 답을 거들면 도움이 되는 경우) suggested_replies 로 2~3개의 "
            "짧고 구체적인 예시 보기를 제공하세요. 이때는 예시가 기억을 돕는 역할을 합니다.\n"
            "- 넓은 질문인지 좁은 질문인지 애매하면, 편향을 피하는 쪽 — 즉 suggested_replies=[] — 을 선택하세요.\n"
