...
### 메인화면 (Home)
- [ ] 메인 지도 비주얼 뒷 배경 누락 복구 (Figma 참조: 연한 청록 톤 백그라운드 텍스처)
- [ ] 지도 위치를 hero 영역에 **수직 중앙 정렬**
- [ ] 지역별 TOP 5를 하단에 추가 배치 — Figma 참조 카드: "부산전체 시민 참여 현황"(좌, 총 참여건수+제보/제안/진단/설문 건수) + "지역별 참여 TOP 5"(우, 순위뱃지+진행바+건수, 1~5위)

### 설문화면 (PC — 스피드 버튼 영역, 설문 진입 카드)
- [ ] "무엇을 도와드릴까요?" 카드에서 우측 스피드 버튼 3개(불편사항 이야기하기/환경 개선 제안하기/우리동네 진단하기)가 좌측 안내문과 **같은 div 안에서 `width:100%`**로 분리된 느낌 나도록 수정 (현재 버튼 영역이 카드와 분리돼 보임)
...
### AI 가상시민 상세 (`MAICitizenDetail.jsx` / `PCAICitizen.jsx`)
- [ ] 상단 이모지가 겹쳐지는 부분 수정 — Figma 참조: 시간대별(07-08/08:30-09/09-14/14-15/17-18/19-20시) 감정 이모지 7개를 가로 1열 + 하단 점선 그래프로 배치. 좁은 폭에서 아이콘 간격이 줄며 겹치는 것으로 추정 — `gap`/`flex-wrap` 대신 `overflow-x:auto` 스크롤 트랙 검토
...
- [ ] 우측 콘텐츠(테마별 지표 카드 그리드 — 청년층 순이동율/고용율/실업율/근로여건 만족도/1인당 GRDP 등) 클릭 시 해당 데이터값이 표시되지 않음 — 대부분 "준비중" 상태인데 데이터 있는 항목(예: 공공도서관 수 "4개관")도 클릭 반응 확인 필요
- [ ] "전체" 버튼 외 다른 카테고리(생활정보 9종) 버튼 클릭 시 전체 버튼 아이콘이 흰색으로 유지되어 사라진 것처럼 보임 — active/inactive 아이콘 색 처리 버그 (Figma는 선택 아이콘만 teal 배경+흰 아이콘, 나머지는 흰 배경+검정 아이콘)
- [ ] 특정 구 클릭 시 조건에 맞는 %값 미표시 — [[project_public_data_page]] 참조, 대부분 지표가 아직 "준비중" mock 상태라 데이터 미비 문제일 가능성 높음. 실데이터 확보된 지표(인구/교통사고/CCTV/PM10 등)부터 우선 확인
...
- [ ] 카테고리 위치 및 리스트 디자인이 Figma와 다름 — Figma 참조: 상단 "지역검색" 드롭다운 + 카테고리 필터 칩(전체/주거/환경/교통/안전/교육 가로 스크롤, teal 액티브) + 하단 시트 리스트(카테고리 태그 2개+제목+좋아요/댓글 수), 하단 네비 위 플로팅 "+ 진단하기" 버튼
...
- [ ] 뒤로가기 버튼 누락 — Figma 참조: 상단 좌측 `<` 뒤로가기 + 우측 "지도보기"/드롭다운 shevron 버튼 쌍
- [ ] 좌측 대시보드 디자인이 Figma와 다름 (제보/제안 동일) — 모바일도 PC와 동일하게 구역별+생활정보 카테고리 rail 구조가 Figma 스펙 (제보 302:5940 계열 통일 컴포넌트, [[project_docs_backlog]] 참조)

### 관리자 — 회원정보 수정 (전문가)
- [ ] 승인 상태를 텍스트("승인")가 아닌 **버튼 형태**(승인/미승인 토글, 액티브=teal 필)로 수정 (관리자에서 클릭으로 회원 상태 변경 가능하도록)
- [ ] 참여현황을 "제안 4건" 같은 텍스트 요약이 아니라 **펼침 리스트**로 표출 — Figma 참조: 제안/제보/설문 섹션별 아코디언, 각 섹션 표(제목/작성자ID/유형/위치, 설문은 상태/답변/수정/메뉴 결과링크 컬럼 추가). 일반 회원도 동일 적용 (현재 누락)

### 관리자 — 제보/제안/설문
- [ ] 디자인 불일치 — 상단 상태 탭 버튼(전체/개선예정/개선중/개선완료 등 pill 버튼 그룹) **제거**하고 "카테고리 선택" 드롭다운 + 하단 서치박스(검색 버튼 포함)로 통일

### 관리자 — 진단관리
- [ ] 등록된 진단을 **수정할 수 있는 버튼 없음** → 상세 관리/위치 추가 등 기능 접근 불가. Figma 참조 화면은 진단관리를 "진단 지역" 단위 리스트(진단지역/진단수/진단인원/등록일 + 메뉴 열의 수정|삭제 링크)로 구성하고 우상단 "진단 지역 추가하기" 버튼 보유 — **현재 코드의 개별 진단 항목 리스트(대상/진단항목/대분류/…)와 화면 성격 자체가 다름**. 진단 항목별 수정을 원하는 건지, 진단 지역(장소) 단위 관리 화면을 신설하라는 건지 요청자 확인 필요

### 관리자 — 공공데이터
- [ ] 시안 상 공공데이터 등록은 **신규 페이지**("공공데이터 등록": 데이터명/영역/편집일/작성일/작성자ID/총 데이터 수+데이터추가 버튼 + "공공데이터 다운로드"/"수정하기" 버튼)인데 현재는 팝업 형태("공공데이터 작성": 테마/지역/지표명/표시값/연도/비고/정렬순서/출처 필드) — 데이터 다운로드·추가 항목이 누락돼 있으므로 페이지 구조를 신규 페이지로 재설계 필요

### 관리자 — 가상시민
- [ ] 편집 화면 디자인이 Figma와 다름 — 현재는 필드가 전부 한 화면에 넓게 나열돼 **가로 스크롤 발생**(이름/나이/성별/한줄인용/태그/카테고리/시민체감언어/관심사/고민/취미/가족/좌우명/꿈꾸는생활/유사시민설명/핵심이슈TOP/시민목소리). Figma는 **탭 구조**(기본정보/데이터 통계/여정지도/정책 신호등)로 나눠 세로 폭 좁은 단일 컬럼 폼으로 구성 — 탭 분리 리팩터링 필요, 가로 스크롤 없애는 게 핵심 조건
