...
**정책결정 필요(미착수)**:
- 관리자 회원 비밀번호 초기화 — 백엔드 스키마(AdminUserUpdate에 PW 없음)부터 확장 필요, 보안 민감 기능이라 별도 결정 후 진행
- 관리자 진단 지도 시각화 — dead code `MapCanvas.jsx`는 용도가 달라(severity 분석용) 재사용 리스크 있음. 드릴다운(리스트)은 완료, 지도 자체는 PCMapCanvas 기반 재구축 필요
- PC 진단맵 "공공/시설물" 대분류 체크박스 — 필터링 자체가 미연결이고, 라벨 체계가 실제 데이터 필드와 안 맞음(정보 및 서비스 매체 등은 어떤 DB 필드에도 대응 안 됨) — 올바른 분류체계 확정 필요
- PC 진단맵 클러스터 클릭 시 지도클릭 버블링 — ref가드+rAF지연으로 대부분 케이스 해결됐으나, 급격한 재줌이 겹치는 일부 케이스에서 여전히 재현됨. 완전 해결은 지도 네이티브 클릭을 끄고 별도 위치선택 모드 도입 필요
